...
def load_mlp_model(model_file):
    """
    Load MLP model from .mlp / .pkl / .joblib file.
    Tries joblib first, then pickle.
    """
    try:
        model = joblib.load(model_file)
        print(f"Loaded MLP model (joblib) from {model_file}", flush=True)
        return model
        ...
        try:
            with open(model_file, "rb") as f:
                model = pickle.load(f)
            print(f"Loaded MLP model (pickle) from {model_file}", flush=True)
            return model
...
def main(percentage):
    ...
    # Paths
    mlp_path = f"./Test_{percentage}/models/MLP/"
    ...
    # Model discovery
    all_files = os.listdir(mlp_path)
    model_files = []
    for filename in all_files:
        if filename.lower().endswith('.mlp'):
    ...
    if len(model_files) == 0:
        print(f"No .mlp files found in {mlp_path}", flush=True)
        sys.exit(1)

    print(f"Found {len(model_files)} MLP files (folds: {[n for n,_ in model_files]})", flush=True)
    ...
    with open(numeralia, "w", encoding="utf-8") as file:
        file.write("MLP Testing Results by Frequency Range\n")
        file.write("=" * 80 + "\n\n")
    ...
    for fold_num, model_filename in model_files:
        ...
        model_file = os.path.join(mlp_path, model_filename)
        mlp_model = load_mlp_model(model_file)
        ...
        y_pred_all = mlp_model.predict(full_data[feature_columns].values.astype(float))
    ...
    with open(numeralia, "a", encoding="utf-8") as f:
        ...
        for cat in sorted(freq_categories):
            ...
            f.write(f" MLP Safer: {s['dunsafe_asafe']:,} ({s['dunsafe_asafe']/td*100:.1f}%)\n")
            f.write(f" MLP Riskier: {s['dsafe_aunsafe']:,} ({s['dsafe_aunsafe']/td*100:.1f}%)\n")
            f.write(f" Net safety improvement: {net:+d}\n")

            if cat in ["Zero (0)", "Very Low (1-4)"]:
                f.write("\nSafety Confusion Matrix:\n")
                f.write(f"Dataset Safe   MLP Safe: {s['dsafe_asafe']:,}   MLP Unsafe: {s['dsafe_aunsafe']:,}\n")
                f.write(f"Dataset Unsafe MLP Safe: {s['dunsafe_asafe']:,}   MLP Unsafe: {s['dunsafe_aunsafe']:,}\n\n")
        ...
        if gt > 0:
            ...
            f.write(f"MLP Safer: {global_disagreement['dunsafe_asafe']:,} ({global_disagreement['dunsafe_asafe']/gt*100:.1f}%)\n")
            f.write(f"MLP Riskier: {global_disagreement['dsafe_aunsafe']:,} ({global_disagreement['dsafe_aunsafe']/gt*100:.1f}%)\n")
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python3 test_mlp_frequency.py <percentage>")
        sys.exit(1)
