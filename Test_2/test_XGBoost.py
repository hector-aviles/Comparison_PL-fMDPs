...
def main(percentage):
    ...
    output = f"./Test_{percentage}/models/XGBoost/Results/testing_numeralia.txt"
    # MODIFIED BLOCK END
    ...
    for i in range(1, num_files + 1):
        ...
        data = data[["action", "curr_lane", "free_E", "free_NE", "free_NW", "free_SE", "free_SW", "free_W", "driver"]]
        
        y_true = encoder.transform(data['action'])
        X_test = data.drop(columns=['action', 'driver'])
        drivers = data['driver']

        model_filename = f"./Test_{percentage}/models/XGBoost/XGBoost_best_{i}.xgboost"
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python test_XGBoost.py <percentage>")
        sys.exit(1)
    main(sys.argv[1])
