...
def main(percentage):
    ...
    output = f"./Test_{percentage}/models/NB/Results/testing_numeralia.txt"
    # MODIFIED BLOCK END
    ...
    for i in range(1, num_files + 1):
        ...
        model_filename = f"./Test_{percentage}/models/NB/NB_{i}.nb"
        ...
        for driver in range(1, 5):
            mask = drivers == driver
            if mask.sum() > 0:
                y_true_driver = y_true[mask]
                y_pred_driver = y_pred[mask]
                d_precision, d_recall, d_f1, d_accuracy = evaluate_model(y_true_driver, y_pred_driver)
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python test_NB.py <percentage>")
        sys.exit(1)
    main(sys.argv[1])
