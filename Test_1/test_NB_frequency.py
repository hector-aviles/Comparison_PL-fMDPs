...
def load_nb_model(model_file):
    """Load a fitted Naive Bayes model from .nb / .pkl / .joblib file"""
    try:
        model = joblib.load(model_file)
        print(f"Loaded Naive Bayes model from {model_file}", flush=True)
        return model
...
def extract_fold_number(filename):
    """Extract fold number from filenames like NB_5.nb, naivebayes_3.nb, naive_bayes_1.nb, etc."""
    match = re.search(r'(?i)(?:NB|naivebayes|naive_bayes)[_*fold]*(\d+)\.(nb|pkl|joblib)', filename)
    if match:
...
def main(percentage):
    ...
    # Paths
    nb_path = f"./Train_{percentage}/models/NB/"
    analysis_file = "./analysis/count_sample_space_auto_with_safe.csv"
    safe_file = "./analysis/no_crashes.csv"
    numeralia = os.path.join(nb_path, "Results", "testing_nb_frequency.txt")
    ...
    # Model discovery
    all_files = os.listdir(nb_path)
    model_files = []
    for filename in all_files:
        if filename.lower().endswith('.nb'):
    ...
    if len(model_files) == 0:
        print(f"No .nb files found in {nb_path}", flush=True)
        sys.exit(1)

    print(f"Found {len(model_files)} NB files (folds: {[n for n,_ in model_files]})", flush=True)
    ...
    with open(numeralia, "w", encoding="utf-8") as file:
        file.write("Naive Bayes (NB) Testing Results by Frequency Range\n")
        file.write("=" * 80 + "\n\n")
    ...
    for fold_num, model_filename in model_files:
        ...
        model_file = os.path.join(nb_path, model_filename)
        nb_model = load_nb_model(model_file)
        if nb_model is None:
            continue
        ...
        y_pred_all = nb_model.predict(full_data[feature_columns].values.astype(float))
        for i, idx in enumerate(full_data.index):
            ...
            nb_safe = check_action_safety(pred_action, state_row, feature_columns, safe_lookup)
            ...
            if ds_safe and nb_safe:
                k = 'dsafe_asafe'
            elif ds_safe and not nb_safe:
                k = 'dsafe_aunsafe'
            elif not ds_safe and nb_safe:
                k = 'dunsafe_asafe'
            else:
                k = 'dunsafe_aunsafe'
    ...
    with open(numeralia, "a", encoding="utf-8") as f:
        ...
        for cat in sorted(freq_categories):
            ...
            f.write(f" NB Safer: {s['dunsafe_asafe']:,} ({s['dunsafe_asafe']/td*100:.1f}%)\n")
            f.write(f" NB Riskier: {s['dsafe_aunsafe']:,} ({s['dsafe_aunsafe']/td*100:.1f}%)\n")
            f.write(f" Net safety improvement: {net:+d}\n")

            if cat in ["Zero (0)", "Very Low (1-4)"]:
                f.write("\nSafety Confusion Matrix:\n")
                f.write(f"Dataset Safe   NB Safe: {s['dsafe_asafe']:,}   NB Unsafe: {s['dsafe_aunsafe']:,}\n")
                f.write(f"Dataset Unsafe NB Safe: {s['dunsafe_asafe']:,}   NB Unsafe: {s['dunsafe_aunsafe']:,}\n\n")
        ...
        if gt > 0:
            ...
            f.write(f"NB Safer: {global_disagreement['dunsafe_asafe']:,} ({global_disagreement['dunsafe_asafe']/gt*100:.1f}%)\n")
            f.write(f"NB Riskier: {global_disagreement['dsafe_aunsafe']:,} ({global_disagreement['dsafe_aunsafe']/gt*100:.1f}%)\n")
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python3 test_nb_frequency.py <percentage>")
        sys.exit(1)
