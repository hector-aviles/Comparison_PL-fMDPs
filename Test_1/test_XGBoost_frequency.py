...
def load_xgb_model(model_file):
    """Load a fitted XGBoost model from .xgboost / .pkl / .joblib file"""
    try:
        model = joblib.load(model_file)
        print(f"Loaded XGBoost model from {model_file}", flush=True)
        return model
...
def extract_fold_number(filename):
    """Extract fold number from filenames like XGBoost_best_13.xgboost, XGBoost_best_1.xgboost, etc."""
    match = re.search(r'(?i)XGBoost_best_(\d+)\.xgboost', filename)
    if match:
        return int(match.group(1))
    return None

# ────────────────────────────────────────────────────────────────
# Safety functions (same as PL-fMDP / CART / MLP)
# ────────────────────────────────────────────────────────────────
...
def main(percentage):
    ...
    # Paths
    xgb_path = f"./Train_{percentage}/models/XGBoost/"
    analysis_file = "./analysis/count_sample_space_auto_with_safe.csv"
    safe_file = "./analysis/no_crashes.csv"
    numeralia = os.path.join(xgb_path, "Results", "testing_xgboost_frequency.txt")
    ...
    # Model discovery
    all_files = os.listdir(xgb_path)
    model_files = []
    for filename in all_files:
        if filename.lower().endswith('.xgboost'):
    ...
    if len(model_files) == 0:
        print(f"No .xgboost files found in {xgb_path}", flush=True)
        sys.exit(1)

    print(f"Found {len(model_files)} XGBoost files (folds: {[n for n,_ in model_files]})", flush=True)
    ...
    with open(numeralia, "w", encoding="utf-8") as file:
        file.write("XGBoost Testing Results by Frequency Range\n")
        file.write("=" * 80 + "\n\n")
    ...
    for fold_num, model_filename in model_files:
        ...
        model_file = os.path.join(xgb_path, model_filename)
        xgb_model = load_xgb_model(model_file)
        ...
        y_pred_all = xgb_model.predict(full_data[feature_columns].values.astype(float))
        for i, idx in enumerate(full_data.index):
            ...
            xgb_safe = check_action_safety(pred_action, state_row, feature_columns, safe_lookup)
            ...
            if ds_safe and xgb_safe:
                k = 'dsafe_asafe'
            elif ds_safe and not xgb_safe:
                k = 'dsafe_aunsafe'
            elif not ds_safe and xgb_safe:
                k = 'dunsafe_asafe'
            else:
                k = 'dunsafe_aunsafe'
    ...
    with open(numeralia, "a", encoding="utf-8") as f:
        ...
        for cat in sorted(freq_categories):
            ...
            f.write(f" XGBoost Safer: {s['dunsafe_asafe']:,} ({s['dunsafe_asafe']/td*100:.1f}%)\n")
            f.write(f" XGBoost Riskier: {s['dsafe_aunsafe']:,} ({s['dsafe_aunsafe']/td*100:.1f}%)\n")
            f.write(f" Net safety improvement: {net:+d}\n")

            if cat in ["Zero (0)", "Very Low (1-4)"]:
                f.write("\nSafety Confusion Matrix:\n")
                f.write(f"Dataset Safe   XGBoost Safe: {s['dsafe_asafe']:,}   XGBoost Unsafe: {s['dsafe_aunsafe']:,}\n")
                f.write(f"Dataset Unsafe XGBoost Safe: {s['dunsafe_asafe']:,}   XGBoost Unsafe: {s['dunsafe_aunsafe']:,}\n\n")
        ...
        if gt > 0:
            ...
            f.write(f"XGBoost Safer: {global_disagreement['dunsafe_asafe']:,} ({global_disagreement['dunsafe_asafe']/gt*100:.1f}%)\n")
            f.write(f"XGBoost Riskier: {global_disagreement['dsafe_aunsafe']:,} ({global_disagreement['dsafe_aunsafe']/gt*100:.1f}%)\n")
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python3 test_xgboost_frequency.py <percentage>")
        sys.exit(1)
