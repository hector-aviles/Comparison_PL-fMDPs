...
def main(percentage):
    ...
    models_path = f"./Train_{percentage}/models/MLP/"
    ...
    time_list = []
    best_params_list = []  # To store best parameters and metrics for each fold
    
    # Define hyperparameter grid
    param_grid = {
        'hidden_layer_sizes': [(8,), (16,), (8, 8), (16, 16)],
    ...
    for i in range(1, num_files + 1):
        ...
        # Initialize variables to track the best model and hyperparameters
        best_model = None
        best_params = None
        best_val_f1 = -1
        ...
        model_filename = f"{models_path}/MLP_{i}.mlp"
        pickle.dump(best_model, open(model_filename, 'wb'))
        
        # Append best parameters and validation metrics for this fold
        best_params_list.append(best_params)
        
        print(f"Best parameters for fold {i}: {best_params}", flush=True)
        print(f"Validation F1 score for fold {i}: {best_val_f1:.4f}", flush=True)
    
    # Calculate average training time
    average_time = statistics.mean(time_list)
    stdev_time = statistics.stdev(time_list)
    
    # Save training numeralia
    ...
    print("All folds processed. Best parameters saved.", flush=True)

if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python3 create_MLP.py <percentage>")
        sys.exit(1)
